...
test("the original icon picker exposes a brand tab and stages the choice until Save", async () => {
  ...
  assert.equal(nodes(picker).find(node => node.type === "Picker").props.pickerStyle, "segmented")
  ...
  assert.deepEqual(saved[3],{brandID:spotify})
...
test("complete-and-save forwards the staged preference only after confirmation", async () => {
  for (const confirmed of [false,true]) {
    ...
    if(completion) {
      ...
      assert.deepEqual(completion[5],{brandID:spotify})
...
test("system auto-match is an explicit system selection and browsing tabs alone does not change either choice", () => {
  const env = editorHarness({ brand: spotify })
  let root = env.render("ItemEditor")
  const props = env.pickerProps(root)
  let picker = env.render("IconPicker", props)
  ...
  assert.equal(nodes(picker).filter(node => node.type === "IconChoiceRow" && node.props.selected).length, 0,
    "a stored system fallback is not selected while the brand choice is active")
...
test("returning from brand selection to the system library preserves the previous concrete SF Symbol", () => {
  const env = editorHarness()
  let root = env.render("ItemEditor")
  const initial = env.pickerProps(root)
  initial.onChanged("creditcard.fill")
  initial.onBrandChanged(spotify)
  root = env.render("ItemEditor")
  const picker = env.render("IconPicker", env.pickerProps(root))
  assert.equal(picker.type, "BrandCatalogView")
  picker.props.onSelect("system")
  root = env.render("ItemEditor")
  assert.equal(env.pickerProps(root).value, "creditcard.fill")
  assert.equal(env.pickerProps(root).brandChoice, "system")
  root.props.toolbar.cancellationAction.props.action()
  assert.ok(!env.events.some(event => event[0] === "save" || event[0] === "complete"))
})
