...
素材来源核对日期：2026-09-08；加载方式更新：2026-09-09；适用版本：2.7.2（PNG 与 2.7.0 相同）。332 个品牌全部有实际图片，共 333 个 PNG 文件（SafePal 有深浅两份）。
...
- 主界面和组件以原彩图案在 40 pt 完成槽内显示；一般圆形为 24 pt，SafePal 为 20 pt，SafePal／Telegram 不额外放大。只裁图片，不缩小完成点击区。只读、缓存、冲突或无法解码时保留 SF Symbol。
