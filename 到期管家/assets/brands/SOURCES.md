...
素材来源核对日期：2026-09-08；加载及显示方式更新：2026-09-09；适用版本：2.8.0（PNG 与 2.7.0 相同）。332 个品牌全部有实际图片，共 333 个 PNG 文件（SafePal 有深浅两份）。
...
- 从 2.8.0 起，主界面、编辑预览、目录和小号组件不再使用强制圆形遮罩。330 个生成图案仍以 1.2 倍、28.8 pt 图片显示在 24 pt 矩形内，恰好只去掉打包新增的 12 px 外圈，完整保留 120 × 120 内容边界内的图案和背景。
- 不抠除原背景、不拉伸或重绘，不修改 PNG、Base64、清单和原文件哈希。圆形源图保持原形，方形图案不再被切去四角；不同原图内部留白仍可能导致视觉大小差异，不声称已逐项做光学尺寸校正。
- 主界面和组件以原彩图案在 40 pt 完成槽内显示；SafePal／Telegram 不额外放大，图像与矩形边界保持 20／24 pt。只移除已知打包留白，不缩小完成点击区。只读、缓存或无法解码时保留 SF Symbol。
...
- 运行时不搜索、不请求 App Store、不下载图标，也不上传事项、SIM 或联系人。品牌按事项明确选择，不由标题自动选择，也不受旧全局模式控制。主界面首屏不读图片，目录每页 32 项且只为出现的行启动异步读取；小号组件只读取当前事项明确选择的品牌，其余尺寸及使用系统图标的事项不读图片。
