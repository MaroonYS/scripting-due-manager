...
export interface LoadedArtwork { image: UIImage; lightBackplate: boolean }
...
export function peekArtwork(id: string | null | undefined, directory: string): LoadedArtwork | null {
  if (!id || !artworkByID(id)) return null
...
export function loadArtwork(id: string | null | undefined, directory: string): Promise<LoadedArtwork | null> {
  if (!id || !artworkByID(id)) return Promise.resolve(null)
...
export async function loadArtworkPage(ids: readonly (string | null | undefined)[], directory: string): Promise<Record<string, LoadedArtwork>> {
  const queue = [...new Set(ids.filter((id): id is string => artworkByID(id) != null))].slice(0, 24)
