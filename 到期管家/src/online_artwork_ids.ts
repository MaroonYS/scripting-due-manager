...
import { icons8MCPLabel, parseIcons8MCPID } from "./icons8_mcp_ids"

export type OnlineArtworkProvider = "fluent" | "icons8"
export interface OnlineArtworkID { provider: OnlineArtworkProvider | "icons8mcp"; name: string }
...
export function parseOnlineArtworkID(id: unknown): OnlineArtworkID | null {
  ...
  const mcp = parseIcons8MCPID(id)
  if (mcp) return { provider: "icons8mcp", name: mcp }
...
export function onlineArtworkLabel(id: string | null | undefined): string | null {
  ...
  return icon ? icon.provider === "fluent" ? `Fluent · ${icon.name.replace(/-/g, " ")}`
    : icon.provider === "icons8mcp" ? icons8MCPLabel(id) : `Icons8 · ${icon.name}` : githubArtworkLabel(id)
}
