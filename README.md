...
紧凑原生风格的 iPhone 到期事项组件，按 Apple 公布的 WidgetKit 尺寸自适应小、中、大版式，支持信用卡账单、订阅、周期事项与 Apple 提醒事项，并可直接从交互式组件完成本期。事项名称会在本机智能匹配 SF Symbols，也可手动选择图标。
...
从 1.2.0 升级时先不要删除手机里的旧脚本；让现有脚本更新到 1.2.1 后运行一次，它会把旧事项迁移到本机持久域。此后更新脚本或刷新数据都无需重新添加事项或桌面组件。
...
- `tests/`：日期、周期、图标匹配、队列补位、幂等完成和提醒事项完成测试。
