...
紧凑原生风格的 iPhone 到期事项组件，按 Apple 公布的 WidgetKit 尺寸自适应小、中、大版式，支持信用卡账单、订阅、周期事项与 Apple 提醒事项。2.1 将中、大号组件统一到小号的视觉结构：使用相同的安全边距和标题轨，完成圆圈与事项标题首行对齐，标题优先占用横向空间，图标与日期、金额或备注收入次行。中号按空间显示 2–3 件，大号显示 5–7 件，完成后后续事项自动补位。完成按钮保留扩大的可点击区域，并以代次（generation）驱动 WidgetKit 内容过渡；已完成事项不会被重新塞回队列。Apple 提醒事项可按一个或多个具体列表筛选；只读列表会显示锁定状态。2.0 按“事项标题 → 精确 List 分类 → List 产品短语 → 备注 → 默认值”离线推断图标，覆盖 126 个 List 图标方向与 1,100+ 个简体、繁体和英文别名，并保持精确边界防误判；备注正文不会写入缓存，只保存推断出的图标提示。手动事项提供 11 种到期类型；内置 130 个本机 SF Symbols，并按 11 个 App Store 常见方向分组，同时保留手动覆盖和旧图标兼容。
