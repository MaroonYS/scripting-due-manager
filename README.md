...
紧凑原生风格的 iPhone 到期事项组件，按 Apple 公布的 WidgetKit 尺寸自适应小、中、大版式，支持信用卡账单、订阅、周期事项与 Apple 提醒事项。三种尺寸统一视觉安全边距，中、大号组件采用 14 pt 横向、11 pt 纵向内容边距；完成按钮扩大了可点击区域，小号组件的圆圈视觉中心与事项标题第一行对齐。完成后以代次（generation）驱动 WidgetKit 内容过渡，已完成事项不会被重新塞回队列。Apple 提醒事项可按一个或多个具体列表筛选；只读列表会显示锁定状态。2.0 按“事项标题 → 精确 List 分类 → List 产品短语 → 备注 → 默认值”离线推断图标，覆盖 126 个 List 图标方向与 1,100+ 个简体、繁体和英文别名，并保持精确边界防误判；备注正文不会写入缓存，只保存推断出的图标提示。手动事项提供 11 种到期类型；内置 130 个本机 SF Symbols，并按 11 个 App Store 常见方向分组，同时保留手动覆盖和旧图标兼容。
